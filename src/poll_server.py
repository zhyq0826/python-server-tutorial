    ...
    while True:
        ...
        for fd, flag in events:
            ...
            if flag & (select.POLLIN | select.POLLPRI):
                ...
                # Client hung up
                print_red('connection {0} is closing ==> '.format(s.getpeername()))
                # Stop listening for input on the connection
                poller.unregister(s)
                s.close()
                ...
                try:
                    ...
                    print_red('connection {0} sending msg ==> '.format(s.getpeername()))
                    s.send(next_msg)
            elif flag & select.POLLERR:
                print_red('connection {0} exceptional ==> '.format(s.getpeername()))
                # Stop listening for input on the connection
                poller.unregister(s)
                s.close()
